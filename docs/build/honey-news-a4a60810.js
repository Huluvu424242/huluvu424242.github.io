...
const workerPromise = import('./fetch-es6.worker-2146cfa0.js').then(m => m.worker);
...
const HoneyNews = class {
  ...
  loadFeedContent() {
    ...
    }), tap((feedData) => console.log("### tap feed data " + feedData.feedtitle)), mergeMap((metaData) => this.mapItemsToPost(metaData)), reduce((posts, item) => posts.concat(item), []), tap((postings) => postings.forEach((post) => console.log("### grouped post: " + post.sortdate))), map((list) => this.sortArray(list)), mergeMap(list => list));
  }
  sortArray(post) {
