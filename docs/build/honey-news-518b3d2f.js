...
const workerPromise = import('./proxy.worker-f4098851.js').then(m => m.worker);
const loadData = /*@__PURE__*/createWorkerProxy(workerPromise, 'stencil.proxy.worker', 'loadData');
...
const HoneyNews = class {
  ...
  async loadFeeds() {
    this.urls = [];
    const cccFeedResponse = await this.loadFeedContent("https://cors-anywhere.herokuapp.com/https://media.ccc.de/news.atom");
    console.log("###\n" + this.getPayload(cccFeedResponse));
    const chanFeedResponse = await this.loadFeedContent("https://cors-anywhere.herokuapp.com/https://a.4cdn.org/a/threads.json");
    console.log("###\n" + this.getPayload(chanFeedResponse));
    const sparkFeedResponse = await this.loadFeedContent("https://codepen.io/spark/feed");
    console.log("###\n" + this.getPayload(sparkFeedResponse));
    const hingkiatFeedResponse = await this.loadFeedContent("https://cors-anywhere.herokuapp.com/https://www.hongkiat.com/blog/feed/");
