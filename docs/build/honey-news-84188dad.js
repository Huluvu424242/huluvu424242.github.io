...
const workerPromise = import('./fetch-es6.worker-f22955e3.js').then(m => m.worker);
...
const HoneyNews = class {
  ...
  loadFeedContent() {
    ...
    return urlObservable.pipe(switchMap((url) => from(loadFeedData(url))));
  }
  async loadFeeds() {
    return new Promise((resolve) => {
      this.loadFeedContent().subscribe((feedData) => this.feeds.push(feedData), (error) => error, () => {
        // rendering trigger
  ...
  render() {
    ...
    return (h(Host, { title: this.getTitleText(), alt: this.getAltText(), role: "button", tabindex: this.hasNoFeeds() ? -1 : this.taborder, class: this.getHostClass(), disabled: this.hasNoFeeds() }, h("ol", null, this.feeds.map((feed) => feed.feed.items.map((item) => h("li", null, "[", feed.feed.title, "]", h("a", { href: "" + item.link, target: "_blank" }, item.title)))))));
  }
