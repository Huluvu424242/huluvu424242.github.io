...
const workerPromise = import('./fetch-es6.worker-cd286548.js').then(m => m.worker);
...
class FeedLoader {
  ...
  loadFeedContent() {
    ...
    return urlObservable.pipe(mergeMap((url) => {
    ...
    }), tap((post) => console.log("### Date: " + PipeOperators.compareDates(post.exaktdate, new Date())
      + "#"
      + post.item.title)), filter((post) => PipeOperators.compareDates(post.exaktdate, new Date()) < 1), reduce((posts, item) => posts.concat(item), []), tap((postings) => postings.forEach((post) => console.log("### unsortiert post: " + post.sortdate))), map((list) => PipeOperators.sortArray(list)), mergeMap(list => list));
  }
}
...
const HoneyNews = class {
  ...
  getUeberschrift(post) {
    const hour = post.exaktdate.getHours();
    if (hour != this.lastHour) {
      this.lastHour = hour;
      return h("h2", null, post.exaktdate.toLocaleDateString() + " " + this.lastHour, " Uhr");
    }
    else {
      return;
    }
  ...
  getPostEntry(post) {
    return h("li", null, "[", post.feedtitle, "](", post.pubdate, ")", h("a", { href: this.getPostLink(post.item), target: "_blank" }, post.item.title));
  }
  render() {
