import { e as consoleError, r as registerInstance, h, f as Host, g as getElement } from './index-44fd41d8.js';
...
const workerPromise = import('./fetch-es6.worker-237c3e88.js').then(m => m.worker);
...
class FeedLoader {
  ...
  getFeedsSingleObserver(feedURLs) {
    ...
    (posts) => PipeOperators.removeDuplicates(posts)), map((posts) => PipeOperators.sortArray(posts)));
  }
  getFeedsPeriodicObserver() {
    return timer(0, 60000 * 5).pipe(mergeMap(() => from(this.getFeedsSingleObserver(this.feedURLs)).pipe(catchError(() => EMPTY))));
  }
}
