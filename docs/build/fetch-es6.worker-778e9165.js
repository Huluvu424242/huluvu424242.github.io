...
function loadFeedDataInternal(url) {
  const backendUrl = "https://huluvu424242.herokuapp.com/feed";
  const queryUrl = backendUrl + "?url=" + url + "&statistic=true";
...
async function loadFeedRanking(url) {
  return from(fetch(url))
    .pipe(catchError(() => EMPTY), switchMap((response) => from(response.json()).pipe(catchError(() => EMPTY))), filter((rawData) => isArray(rawData)), map((items) => {
    const statistics = [];
    items.forEach((item) => {
      const statistic = {
